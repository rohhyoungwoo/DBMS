...
-- 학생 테이블 생성
-- CREATE TABLE 테이블명 {
--	컬럼명 데이터타입-문자(VARCHAR@, CHAR), 숫자(NUMBER), 날짜(DATE)
-- );
...
SELECT STUDENT_NUMBER, STUDENT_NAME, STUDENT_MATH, STUDENT_ENG, STUDENT_KOR, STUDENT_GRADE 
FROM TBL_STUDENT
ORDER BY STUDENT_NUMBER; 

--INSERT INTO TBL_STUDENT(STUDENT_GRADE)
--VALUES (STUDENT_MATH + STUDENT_ENG + STUDENT_KOR)/3;
...
FROM TBL_STUDENT;
-- NVL2(컬럼명, 값1, 값2) 값1 = 숫자 불가능 문자 타입이 우선됨
...
-- COALESCE(값1, 값2, 값3) : NULL이 아닌 첫번째 값을 반환 (첫번째로 NULL이 아닌 값 반환)
...
UPDATE TBL_STUDENT
SET STUDENT_GRADE = 'C'
WHERE (STUDENT_MATH + STUDENT_ENG + STUDENT_KOR) / 3 >= 50 AND 
(STUDENT_MATH + STUDENT_ENG + STUDENT_KOR) / 3 < 80;
...
(STUDENT_GRADE = 'B' OR STUDENT_GRADE = 'A');
