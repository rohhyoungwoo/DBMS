...
CREATE TABLE TBL_BOOKPRICE(
	BP_ID NUMBER,
	BP_PRICE NUMBER,
	CONSTRAINT OP_PK PRIMARY KEY(BP_ID)
);
...
-- 두 개의 테이블에서 일치하는 컬럼을 확인 후 ON절에 추가
...
SELECT TB.BOOK_TITLE, TB.BOOK_ID, TM.MEM_ID ,TM.MEM_NAME 
FROM TBL_RENTAL tr 
...
-- 책 대여일-반납일 사이에 특정 날짜가 포함되어있는지 확인 조회
...
-- FROM : 테이블이나 뷰읟 ㅔ이터 가죠오기
-- ON : JOIN절의 조건 평가
-- JOIN : 지정된 조거에 따라 두 개 이상의 테이블을 조인
-- WHERE : 조인 결과에서 조건에 맞는 행을 필터링
-- GROUP BY : 데이터 그룹화(집계함수와 주로사용)
...
-- BOOKS 테이블 기준 왼쪽 외부조인을 수행하고 BOOKPRICE 테이블의 데이터를 포함(왼쪽 테이블의 데이터가 없어도 결과에 포함됨)
