...
--VALUES(1, '김철수', 17, 5);
-- 오류발생! FK인 SCHOOL_NUMBER에 상위 테이블에 존재하지 않는 값을 저장하려하면 오류 발생

-- 제약조건 UK(UINQUE) : 고유한 값이지만 NULL도 허용한다
-- 학생 테이블 STU
...
-- STU_GENDER CHAR(1) DEFAULT 'M' NOT NULL CHECK(STR_GENGER = 'M' OR STU_GENDER='W')
-- DEFAULT 'M' : 데이터가 돌아오지 않으면 무조건 'M'이 기본값으로 들어가게 설정한다
-- NOT NULL : 디폴트값을 W로 설정했으니 NULL 값을 강제로 넣는 것을 막기위해 사용하는 제약조건
-- CHECK(STU_GENDER = 'M' OR STU_GENDER = 'W') 이 컬럼의 값은 M 또는 W만 들여오도록 하는 제약 조건
...
CREATE TABLE TBL_PERSON(
	PERSON_NUMBER NUMBER CONSTRAINT PK_PERSON PRIMARY KEY,
	PERSON_NAME VARCHAR2(1000),
	PERSON_AGE NUMBER,
	PERSON_PHONE VARCHAR2(1000),
	PERSON_ADDRESS VARCHAR2(1000)
);
...
CREATE TABLE TBL_BOOK(
	...
	BOOK_GENRE CHAR(1) DEFAULT 'A' NOT NULL 
	CONSTRAINT CHECK_GEN CHECK(BOOK_GENRE = 'H' OR BOOK_GENRE = 'I' OR BOOK_GENRE = 'R'),
	PERSON_NUMBER NUMBER,
	CONSTRAINT PK_BOOK PRIMARY KEY(BOOK_NUMBER),
	CONSTRAINT FK_BOOK FOREIGN KEY(PERSON_NUMBER) REFERENCES TBL_PERSON(PERSON_NUMBER) 
);

SELECT * FROM TBL_BOOK;
-- 장르 테이블
--CREATE TABLE TBL_GENRE(
--	GENRE_HUMAN VARCHAR2(1000),
--	GENRE_IT VARCHAR2(1000),
--	GENRE_REASON VARCHAR2(1000),
--	BOOK_NUMBER NUMBER,
--	CONSTRAINT FK_GENRE FOREIGN KEY(BOOK_NUMBER) REFERENCES TBL_BOOK(BOOK_NUMBER)
--);
